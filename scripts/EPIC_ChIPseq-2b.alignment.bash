...
path_reference=/biodata/dep_coupland/common/TAIR10
...
bwa index -p ${path_reference}/TAIR10bwaidx -a bwtsw ${path_reference}/TAIR10_all.fas
...
col_num=$(awk -F '\t' 'NR==1{for(i=1;i<=NF;i++)if($i=="file"){print i;exit}}' "${working_directory}/${file}")
...
do
    # Execute the line
    echo ${line}
    bwa mem -t 4 ${path_reference}/TAIR10bwaidx ${path_reads}/${line} | samtools view -b - | samtools sort - -o ${path_alignments}/${line%fastq.gz}.bam
    samtools index ${path_alignments}/${line%fastq.gz}.bam
done
